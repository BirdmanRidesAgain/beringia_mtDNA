...
| `annotation_success` | MitoZ run under `annotation/<suffix>/`: `success` = at least one non-zero output file; `failed` = output files exist but all are 0 bytes; `not_attempted` = no annotation outputs on disk. |
